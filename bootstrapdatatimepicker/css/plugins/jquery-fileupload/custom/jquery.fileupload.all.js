...
(function (factory) {
    ...
    $.widget('blueimp.fileupload', $.blueimp.fileupload, {
        ...
        options: {
            ...
            // Error and info messages:
            messages: {
                maxNumberOfFiles: '不能超过最大上传个数',
                acceptFileTypes: '文件格式有误',
                maxFileSize: '文件过大',
                minFileSize: '文件太小'
            }
        },
    ...
    $.widget('blueimp.fileupload', $.blueimp.fileupload, {
        ...
        options: {
            ...
            add: function (e, data) {
                ...
                // 选择文件个数限制
                if (options.beforeAdd && !options.beforeAdd(data.files) ) {
                    return;
                }
            ...
            // Callback for successful uploads:
                ...
                // that.options.doneHook && that.options.doneHook(e, data);
                if(that.options.doneHook) {
                    return that.options.doneHook(e, data);
                }
                if(that.options.hiddenInputId){
                    var $hiddenInput = $('#'+that.options.hiddenInputId);
                    // data.result.files = data.result.data || data.result.files;
                    $.each(files, function (index, file) {
                        if($hiddenInput.val()){
                            $hiddenInput.val($hiddenInput.val() + "|" + file.url);
                        }else{
                    ...
                    removeNode = function () {
                        that._transition(data.context).done(
                            function () {
                                ...
                                that.options.deleteHook && that.options.deleteHook();
                                if(that.options.hiddenInputId){
                                    //移除后的钩子，重新设置隐藏域的值
                                    var downloads = $("." + that.options.downloadTemplateId),
                                        $hiddenInput = $("#"+that.options.hiddenInputId);
                                    //清空隐藏域
                                    $hiddenInput.val('');
                                    $.each(downloads, function (index, tr) {
                                        tr = $(tr);
                                        if($hiddenInput.val()){
                                            $hiddenInput.val($hiddenInput.val() + "|" + tr.find("span.preview a").attr("href"));
                                        }else{
                                            $hiddenInput.val(tr.find("span.preview a").attr("href"));
                                        }
                                    });
                                }
                            }
        ...
        _initSpecialOptions: function () {
            ...
            //window.isSingleUpload为true时，无需初始化多文件上传模板
            if(!window.isSingleUpload)
                this._initTemplates();
        },
